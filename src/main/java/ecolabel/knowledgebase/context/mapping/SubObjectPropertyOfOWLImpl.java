 ...
 */
public class SubObjectPropertyOfOWLImpl extends MappingOWLImpl {
	...
	public void parseImpl(Node nl, OWLOntologyManager owlmgr, LocalOWLModule lom) {
		...
		OWLObjectProperty tempObjectPropertyB = df.getOWLObjectProperty(IRI.create(tempEleB.getAttribute("IRI")));
	...
	public void renderMappingImpl(Node nl, BinaryMappingComponent bmc) {
		...
		bmc.mappingType.setSelectedItem("SubObjectPropertyOf");//xd set the selected mapping type 
		bmc.lbMappingType.setText("SubObjectPropertyOf");
	...
	public Element buildRealBinaryMappingImpl(BinaryMappingComponent bmc, Document doc) {
		...
		eleObjectProperty2.setAttribute("IRI", getEntityIRIFromTooltip(bmc.tfHead.getToolTipText()));
		Element eleObjectProperty3 = doc.createElement("ObjectProperty");
		eleObjectProperty3.setAttribute("IRI", getEntityIRIFromTooltip(bmc.tfTail.getToolTipText()));
